...
class Brewworks < Formula
  ...
  def install
    ...
    (log_dir/".gitkeep").write <<~EOS
    EOS

    (tmp_dir/".gitkeep").write <<~EOS
    EOS

    (config_dir"/php-fpm.conf").write <<~EOS
      [global]
      daemonize = no
    ...
    if PORTS[:redis] > 0
      (config_dir"/redis.conf").write <<~EOS
        port #{PORTS[:redis]}
    ...
    if PORTS[:memcached] > 0
      (config_dir"/memcached.conf").write <<~EOS
        -p #{PORTS[:memcached]}
        -l 127.0.0.1
    ...
    (config_dir"/nginx.conf").write <<~EOS
      server {
        listen #{PORTS[:nginx]};
    ...
    (config_dir"/httpd.conf").write <<~EOS
      Listen #{PORTS[:httpd]}
    ...
    (config_dir"/nginx_main.conf").write <<~EOS
      events {
    ...
    (project_dir/"env.sh").write <<~EOS
      # Change the settings below according to your project requirements:
      # - PROJECT_NAME: The name of your project
      # - PHP_VERSION: The version of PHP to use
      # - MYSQL_VERSION: The version of MySQL to use
      # - DEPENDENCIES: The list of dependencies needed for the project
      # - PORTS: The ports for each service
      # - PHP_EXTENSIONS: The PHP extensions to be used    # Change the settings below according to your project requirements:
          # - PROJECT_NAME: The name of your project
          # - PHP_VERSION: The version of PHP to use
          # - MYSQL_VERSION: The version of MySQL to use
          # - DEPENDENCIES: The list of dependencies needed for the project
          # - PORTS: The ports for each service
          # - PHP_EXTENSIONS: The PHP extensions to be used    EOS
